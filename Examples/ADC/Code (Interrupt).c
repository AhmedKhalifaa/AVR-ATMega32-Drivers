...
void ADC_TestInterrupt(void){
	uint16_t temperature = 0;
	ADC_ReadResult(&LM35_Sensor, &temperature);
	if(NULL != &LM35_Sensor){
		if(temperature>=640){
			LCD_4BitWriteString(&My_LCD, "Too Hot!");
		}
		else if(temperature<384){
			LCD_4BitWriteString(&My_LCD, "Too Cold!");
		}
		else{
			LCD_4BitWriteString(&My_LCD, "Just Right + Cozy!");
		}
	}
	_delay_ms(1000);
	LCD_4BitClearDisplay(&My_LCD);
	CLR_BIT(ADC_ADCSRA_REG,ADC_ADCSRA_ADIF_BIT);
	ADC_StartConversion();
}
...
int main(){
	...
	ADC_SetCallBack(&LM35_Sensor, ADC_TestInterrupt);
	ADC_EnableInterrupt();
	ADC_StartConversion();
	while(1){
